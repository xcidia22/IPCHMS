<?php
    ...
    $sApril = $dbh->prepare("SELECT COUNT(*) as total FROM `student_school_matching` WHERE `month` = 'Apr' && `year` = '$year' && traveldest = 0 and `accept` = 1");
    $sApril->execute();
    ...
    $tApril = $dbh->prepare("SELECT COUNT(*) as total FROM `student_school_matching` WHERE `month` = 'Apr' && `year` = '$year' && courseid = 0 and `accept` = 1");
    $tApril->execute();
